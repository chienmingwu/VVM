...
#if defined (MPI)
      USE domain_decomposition, only : mpi_real_kind, mpierr
#endif
...
! Trace gas profiles from fort.91
      ...
      REAL (KIND = real_kind), DIMENSION(km+1) :: &
          tmppres, tmpTrace
      REAL (KIND = real_kind), DIMENSION(km+2, nTraceGases) :: &
          trpath
      REAL (KIND = real_kind), DIMENSION(nTraceGases,nPress) :: &
          trace
      REAL (KIND = real_kind), DIMENSION(km+2) :: &
          tmppresi

      REAL (KIND = dbl_kind) :: &
          plow, pupp, pmid, wgtlow, wgtupp, godp
...
#if defined (MPI)
      CALL MPI_BCAST( pmls,     nzls, mpi_real_kind, 0, MPI_COMM_WORLD, mpierr)
      CALL MPI_BCAST( o3MLS,    nzls, mpi_real_kind, 0, MPI_COMM_WORLD, mpierr)
      CALL MPI_BCAST( co2MLS,   nzls, mpi_real_kind, 0, MPI_COMM_WORLD, mpierr)
      CALL MPI_BCAST( ch4MLS,   nzls, mpi_real_kind, 0, MPI_COMM_WORLD, mpierr)
      CALL MPI_BCAST( n2oMLS,   nzls, mpi_real_kind, 0, MPI_COMM_WORLD, mpierr)
      CALL MPI_BCAST( o2MLS,    nzls, mpi_real_kind, 0, MPI_COMM_WORLD, mpierr)
      CALL MPI_BCAST( cfc11MLS, nzls, mpi_real_kind, 0, MPI_COMM_WORLD, mpierr)
      CALL MPI_BCAST( cfc12MLS, nzls, mpi_real_kind, 0, MPI_COMM_WORLD, mpierr)
      CALL MPI_BCAST( cfc22MLS, nzls, mpi_real_kind, 0, MPI_COMM_WORLD, mpierr)
      CALL MPI_BCAST( ccl4MLS,  nzls, mpi_real_kind, 0, MPI_COMM_WORLD, mpierr)
#endif
...
! add a level for the overlying atmosphere.
      ...
      tmppresi(km+2) = MIN(1.e-4_real_kind,0.25*tmppres(km+1)) ! near-zero pressure at top of extra laye
